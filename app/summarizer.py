import logging
import json
from datetime import date, datetime  # keep ONLY this import form
...
def generate_summaries(reviews, model=DEFAULT_MODEL, output_response=False):
    if not reviews:
        return "No new reviews.", "No new reviews."

    review_texts = "\n".join([f"- {r.get('review_comment','')}" for r in reviews if r.get('review_comment')])
...
"""
    ...
    if output_response:
        try:
            with open("/tmp/full_response_wins.json", "w", encoding="utf-8") as f:
                f.write(json.dumps(wins_response.model_dump(), indent=2))
            with open("/tmp/full_response_opps.json", "w", encoding="utf-8") as f:
            ...
            logger.exception("Error writing temp files: %s", e)
...
def load_summaries(start_date, end_date, wins, opps, review_count):
    ...
    # SINGLE dict row
    row = {
        "week_start": start_date,                 # date or 'YYYY-MM-DD'
        "week_end": end_date,                     # date or 'YYYY-MM-DD'
        "wins_summary": wins,                     # STRING
        "opps_summary": opps,                     # STRING
        "review_count": review_count,             # INTEGER
        "insert_timestamp_utc": datetime.utcnow().isoformat() + "Z"
    }

    logger.info(f"Loading generated summary into {table_id}...")
    errors = bq_client.insert_rows_json(table_id, [_deep_json_safe(row)])
    if errors:
...
if __name__ == "__main__":
    ...
    wins, opps = generate_summaries(reviews, output_response=True)
    print(wins, opps)
